...
export async function invalidatePostCache(
  ...
  // look up the organization slug from the ID if an ID is passed
  const cachePaths = [
    `/api/public/v1/${organizationIdOrSlug}/posts/${postIdOrSlug}`,
    `/api/public/v1/${organizationIdOrSlug}/posts`, // Also invalidate list
  ];
...
export async function invalidateTaxonomyCache(
  ...
  if (termSlug && taxonomySlug) {
    ...
    cachePaths.push(
      `/api/public/v1/${organizationIdOrSlug}/taxonomies/${taxonomySlug}/${termSlug}/posts`
    );
    cachePaths.push(`/api/public/v1/${organizationIdOrSlug}/posts`); // Also invalidate main list
  }

  if (taxonomySlug) {
    // Invalidate taxonomy detail
    cachePaths.push(
      `/api/public/v1/${organizationIdOrSlug}/taxonomies/${taxonomySlug}`
    );
  }

  // Always invalidate main posts list as taxonomies affect post filtering
  cachePaths.push(`/api/public/v1/${organizationIdOrSlug}/posts`);
