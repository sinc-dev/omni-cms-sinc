/**
 * Add SINCUNI and SINCEDU Organizations with API Keys
 * 
...
const ORGANIZATIONS = [
  {
    name: 'SINCUNI',
    slug: 'sincuni',
    description: 'SINCUNI - Organization for SINCUNI',
  },
  {
    name: 'SINCEDU',
    slug: 'sincedu',
    description: 'SINCEDU - Organization for SINCEDU',
  },
];

// API key scopes for content management (full access)
...
async function saveApiKeys(apiKeys) {
  const keysFile = path.join(__dirname, '../sincuni-sincedu-api-keys.json');
  const envFile = path.join(__dirname, '../.env.sincuni-sincedu-api-keys');
  ...
  const envContent = [
    `# API Keys for SINCUNI and SINCEDU Organizations`,
    ...
    ``,
    `# Usage example:`,
    `# export SINCUNI_API_KEY="${apiKeys.find(k => k.organizationName === 'SINCUNI')?.key || 'your-key-here'}"`,
    `# export SINCEDU_API_KEY="${apiKeys.find(k => k.organizationName === 'SINCEDU')?.key || 'your-key-here'}"`,
  ].join('\n');
  ...
  try {
    ...
    if (!gitignore.includes('sincuni-sincedu-api-keys.json') || !gitignore.includes('.env.sincuni-sincedu-api-keys')) {
      await fs.appendFile(gitignorePath, '\n# SINCUNI/SINCEDU API Keys\nsincuni-sincedu-api-keys.json\n.env.sincuni-sincedu-api-keys\n');
      console.log(`✓ Updated .gitignore`);
    }
  } catch (error) {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, '# SINCUNI/SINCEDU API Keys\nsincuni-sincedu-api-keys.json\n.env.sincuni-sincedu-api-keys\n');
...
async function main() {
  ...
  console.log('Add SINCUNI and SINCEDU Organizations with API Keys');
  console.log('='.repeat(60));
  console.log(`\nAPI URL: ${API_URL}`);
  ...
  try {
    ...
    console.error('  ADMIN_API_KEY=your-jwt-token node scripts/add-sincuni-sincedu-organizations.js');
    ...
    console.error('  node scripts/add-sincuni-sincedu-organizations.js');
