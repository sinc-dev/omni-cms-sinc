 ...
 */
app.get(
  '/:orgId/schema/:objectType',
  authMiddleware,
  orgAccessMiddleware,
  permissionMiddleware('schema:read'),
  async (c) => {
    const objectType = c.req.param('objectType');
    
    if (!objectType) {
      return c.json(Errors.badRequest('objectType parameter is required'), 400);
    }

    try {
      switch (objectType) {
        case 'posts':
          return c.json(successResponse(getPostsSchema()));
        case 'media':
          return c.json(successResponse(getMediaSchema()));
        case 'users':
          return c.json(successResponse(getUsersSchema()));
        case 'taxonomies':
          return c.json(successResponse(getTaxonomiesSchema()));
        default:
          return c.json(Errors.notFound(`Schema for object type '${objectType}' not found`), 404);
      }
    } catch (error) {
      console.error(`Error fetching schema for ${objectType}:`, error);
      return c.json(Errors.serverError(`Failed to fetch schema for ${objectType}`), 500);
    }
  }
);
